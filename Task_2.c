...
int main(int argc, char* argv[]) {
	...
	// scanf("%d", &temp);
	char* individualToken = '\0';
	int argument[] = {};
	individualToken = strtok(argv[1], ",");
	while (individualToken!= NULL){
		insert_node(&treePtr, atoi(individualToken));
...
{
	if (*treePtr != NULL){
		delete_tree(&((*treePtr)->leftPtr)); // looks left
		delete_tree(&((*treePtr)->rightPtr)); // looks right
		free(*treePtr); // visits the node
